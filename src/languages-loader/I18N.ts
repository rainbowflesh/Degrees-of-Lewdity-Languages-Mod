...
export class ModI18N {
  ...
  _: any = window.modUtils.getLodash();
  logger: LogWrapper;
  ...
  patchVersionString() {
    const selfZip: ModZipReader | undefined = this.modSC2DataManager.getModLoader().getModZip(packageJson.name);
    if (selfZip) {
  ...
  async readZipStream() {
    ...
    const selfZip = this.modSC2DataManager.getModLoader().getModZip(packageJson.name);
  ...
  private async parseOriginalZip(selfZip: any) {
    ...
    const stream: JSZipStreamHelper<string> = selfZip.zip.file("translates.json")?.internalStream("string");
  ...
  async readZipSelf() {
    ...
    if (selfZip) {
      ...
      const i18nJsonS = await selfZip.zip
        .file("translates.json")
        ?.async("string")
        .catch((err: any) => {
          console.error(err);
          return undefined;
