...
class ModI18NTypeB_OutputTextMatcher {
  ...
  constructor(public mt: TypeBOutputText[]) {
...
class ModI18NTypeB_PassageMatcher {
  constructor(public mt: TypeBInputStoryScript[]) {
  ...
  replacePassageContentWithHintIndex(passageName: string, passageContent: string) {
    ...
    if (pp) {
      ...
      // console.log(s);
      for (const v of pp) {
        let d = ModI18NTypeB_PassageMatcher.tryReplaceStringFuzzyWithHintIndexComp(
          textArray,
          s,
          v,
          passageName,
  ...
  static tryReplaceStringFuzzyWithHintIndex(
      ...
      try {
        ...
        if (mm) {
          ...
          logger.error(
            ...
            [v.from],
            " in ",
            [passageNameOrFileName],
            " at ",
            [v.pos],
            " in ",
            [s.substring(v.pos - 10, v.pos + v.from.length + 10)]
          );
        }
        re = undefined;
      } catch (e) {
  ...
  static tryReplaceStringFuzzyWithHintIndexComp(
    textArray: Array<string>,
    s: string,
    v: {
      from: string;
  ...
  static tryReplaceStringFuzzyWithHint(
    s: string,
    v: {
      from: string;
      to: string;
...
class ModI18NTypeB_JsCssMatcher {
  constructor(public OutputText: TypeBOutputText[], public InputStoryScript: TypeBInputStoryScript[]) {
